...
**Date:** 2026-07-27 (Media inspection Save As, resizable sidebar, and visible-name theme ordering)

**Scope:** added a wired Save As action to the Media Studio detail dialog, consolidated card/detail/single-export behavior through one renderer callback and the existing secure preload/main-process save routes, added a persisted and accessible desktop sidebar resize separator, and sorted the combined Theme Maker list by final visible label.

**Implementation:** generated-media records call `desktopFiles.saveGeneratedMedia`; legacy URL/data/custom-protocol sources are resolved to bytes and call `desktopMedia.saveMediaDataUrl`, with native cancellation returning silently. Sidebar persistence schema v14 stores `sidebarOpen` and a separately clamped `sidebarWidth` (220–480 px, default 256 px, collapsed 60 px); pointer movement updates only the CSS custom property and persists once on completion, while keyboard, Home/End, double-click reset, collapse/expand restoration, pointer capture, and separator ARIA are covered. Theme options use a non-mutating `Intl.Collator` helper (`sensitivity: "base"`, `numeric: true`); duplicate-ID overrides occur before sorting and the action-like Custom Theme entry remains last. New user-visible media/sidebar strings are catalogued, with fallback-safe entries propagated to every locale; native linguistic review status is unchanged.

**Validation under Node 22.13.1 / npm 10.9.2:** zero-warning ESLint and both TypeScript projects passed; focused acceptance sweep passed 123/123; workspace contracts passed 205/205; storage/privacy passed 41/41; Media Studio, theme-token, custom-protocol, Markdown-link, localization, hard-coded-string, and production-build gates passed. The first `verify:i18n` run identified interpolation-less generated missing markers for three new parameterized strings; marker variables were corrected and the gate passed. `npm run test:ci` passed its 60 server tests, then the long Electron segment reported 835/836 after `backupCrypto.test.ts` timed out; the exact isolated file immediately passed 4/4 in 5.50 seconds, separating the aggregate timeout from this change. `verify:no-native-dialogs` reports the existing approved directory chooser at `electron/services/generatedMediaExport.ts:177`; this diff does not modify that service. `npm ci` completed under the ambient Node 26/npm 11 first and reported the repository's existing 16 high-severity transitive advisories; all implementation validation used the required Node 22/npm 10 toolchain.

**Manual QA:** automated rendered jsdom coverage verifies action order, focus accessibility, dialog retention, native route selection/cancellation, sidebar drag/keyboard/reset/collapse behavior, and visible theme order. A headed packaged Electron session, real macOS Save dialog write, restart persistence, and screenshot capture were not run in this terminal session and remain external verification under `VF-VERIFY-005`; no manual pass is claimed.
