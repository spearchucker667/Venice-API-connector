...
**Date:** 2026-07-28 (Packaged macOS media-save/sidebar verification and dependency-advisory triage)

**Scope:** exercised the preceding Media Studio Save As and sidebar persistence implementation in a real unsigned packaged Apple-silicon build, then classified the repository's 16 high-severity npm advisories by production reachability and dependency ownership.

**Verified behavior:** `npm run dist:mac:arm64` produced `release/mac-arm64/Venice Forge.app`; the packaged Electron smoke test passed. In the packaged UI, the detail header rendered Favorite, Save As…, Delete, and Close in order. Save As opened the native macOS dialog with `task-result--video_generation.mp4`, wrote a 9,381,682-byte MP4 with an `ftypisom` signature, and left the detail dialog open. Cancelling a second Save As left the dialog open without an error or success state. A sidebar drag persisted 472 px across a real app close/relaunch; collapse hid the separator, expansion restored 472 px, and double-click reset the persisted preference to 256 px. The session changed no application media records.
...
**Remaining verification boundary:** this headed run proves one existing packaged video and sidebar state on unsigned Apple silicon. It does not prove generated/legacy image or audio saves, signed/notarized distribution behavior, Intel macOS, Windows, paid-provider generation, assistive technologies, or cross-device recovery. Those portions remain under `VF-VERIFY-005`.
