...
Direct source-image web matching is not currently exposed. The former potential-source action sent an editable descriptive text query rather than image bytes and is disabled so it cannot be mistaken for reverse-image matching. Historical local records remain labeled as legacy text-query results. Safe diagnostics and Traffic Inspector exports exclude raw image bytes, base64 media, complete prompts, credentials, and local absolute paths.
