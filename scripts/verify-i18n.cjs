...
function flattenTree(obj, prefix = '', out = []) {
  for (const [k, v] of Object.entries(obj)) {
    const keyPath = prefix ? `${prefix}.${k}` : k;
    if (v && typeof v === 'object' && !Array.isArray(v)) flattenTree(v, keyPath, out);
...
function runVerification({
  ...
  for (const locale of activeLocales) {
    ...
    if (!fs.existsSync(dir)) {
      errors.push(`Missing resource directory for locale '${locale}' at ${dir}`);
    }
  }
    ...
    try {
      ...
      for (const [, v] of flattenTree(parsed)) {
        ...
        if (SENTINEL_PATTERN.test(v)) {
          errors.push(`en-US canonical seed for '${ns}' contains sentinel value: "${v}"`);
        }
        if (MISSING_MARKER_PATTERN.test(v)) {
          errors.push(`en-US canonical seed for '${ns}' contains __MISSING__ marker: "${v}"`);
        }
      }
    } catch (err) {
      errors.push(`Failed to parse en-US namespace file ${file}: ${err.message}`);
    }
  }
  ...
  for (const locale of activeLocales) {
    ...
    for (const ns of activeNamespaces) {
      ...
      for (const k of Object.keys(enKeys)) {
        if (!(k in locKeys)) {
          ...
          if (enVars.join('|') !== locVars.join('|')) {
            errors.push(`Locale '${locale}' key '${ns}:${k}' interpolation mismatch. Expected [${enVars.join(', ')}], got [${locVars.join(', ')}]`);
          }
          ...
          if (SENTINEL_PATTERN.test(locVal) || MISSING_MARKER_PATTERN.test(locVal)) {
            ...
            errors.push(`Locale '${locale}' key '${ns}:${k}' contains unapproved untranslated English text: "${enVal}"`);
          }
        }
      }
      ...
      for (const k of Object.keys(locKeys)) {
        if (!(k in enKeys)) {
          errors.push(`Locale '${locale}' has orphan key '${ns}:${k}' not found in en-US`);
        }
      }
    }

    const pct = localeTotal > 0 ? parseFloat(((localeTranslated / localeTotal) * 100).toFixed(1)) : 0;
  ...
  if (activeDocs.length > 0) {
    for (const locale of activeLocales) {
      ...
      if (!fs.existsSync(locDocsDir)) {
        errors.push(`Missing documentation directory for locale '${locale}' at ${locDocsDir}`);
      ...
      for (const doc of activeDocs) {
        ...
        if (fs.existsSync(docPath)) {
          ...
          errors.push(`Missing localized doc '${doc}' for locale '${locale}' at ${docPath}`);
        }
      }
  ...
  if (writeStatus) {
    ...
    fs.writeFileSync(targetPath, JSON.stringify(statusMetadata, null, 2) + '\n', 'utf8');
  }
...
if (require.main === module) {
  ...
  const result = runVerification({ strict, allowSentinels, allowMissingMarkers });
