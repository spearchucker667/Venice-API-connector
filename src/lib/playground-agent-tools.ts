...
function isValidId(value: unknown): value is string {
  return (
    typeof value === "string" && value.length <= MAX_ID_LEN && ID_RE.test(value)
  );
}
...
const PARAM_SCHEMA_PROPERTIES: Record<
  string,
...
const TOOLS: ToolDefinition[] = [
  {
    ...
    function: {
      name: "clear",
      // i18n-allow-next-line: model-facing agent tool schema, not application presentation
      description:
        "Remove all nodes and edges. Use when starting a fresh workflow.", // i18n-allow: model-facing agent tool schema, not application presentation
      parameters: {
        type: "object",
        properties: {},
      ...
      parameters: {
        ...
        properties: {
          ...
          id: {
            ...
            description:
              "Optional explicit id. If omitted, a uuid is generated. Use this id in subsequent connect/set_params calls.",
          },
          params: {
            type: "object",
            // i18n-allow-next-line: model-facing agent tool schema, not application presentation
            description:
              "Node-specific params (model, prompt, temperature, etc). See system prompt for the full schema per node type.", // i18n-allow: model-facing agent tool schema, not application presentation
      ...
      parameters: {
        ...
        properties: {
          ...
          source: {
            ...
            pattern: "^[a-zA-Z0-9_-]{1,64}$",
            maxLength: MAX_ID_LEN,
            description: "Source node id.", // i18n-allow: model-facing agent tool schema, not application presentation
          },
          // i18n-allow-next-line: model-facing agent tool schema, not application presentation
          target: {
            type: "string",
            pattern: "^[a-zA-Z0-9_-]{1,64}$",
            maxLength: MAX_ID_LEN,
            description: "Target node id.", // i18n-allow: model-facing agent tool schema, not application presentation
          },
        },
  ...
  },
  {
    ...
    function: {
      ...
      // i18n-allow-next-line: model-facing agent tool schema, not application presentation
      description:
        "Update params on an existing node. Merged with current params.", // i18n-allow: model-facing agent tool schema, not application presentation
      parameters: {
        type: "object",
        properties: {
          id: {
    ...
    .map((s) => {
      const params = s.params
        .map((p) => {
          ...
          if (p.enumValues)
            bits.push(`one of [${p.enumValues.filter(Boolean).join(", ")}]`);
...
export interface RunOptions {
  ...
  applyPatch: (
    patch: WorkflowPatch,
  ) => { ok: true; id?: string; edge_id?: string } | { error: string };
...
function describeDraft(draft: {
  ...
  const ns = draft.nodes
    .map((n) => `  - ${n.id} [${n.data.nodeType}]`)
    .join("\n");
  const es =
    draft.edges.map((e) => `  - ${e.source} → ${e.target}`).join("\n") ||
    "  (none)";
  ...
  try {
    switch (name) {
      ...
      case "add_node": {
        ...
        if (!node_type || !(node_type in NODE_SCHEMAS))
          return { error: `Unknown node_type: ${String(node_type)}` };
        if (typeof args.id === "string" && args.id && !isValidId(args.id))
          return { error: safeIdError("node id") };
        ...
        const allowed = new Set([
          "model",
          "prompt",
          ...schema.params.map((p) => p.name),
        ]);
        const params: Record<string, unknown> = {};
        for (const [k, v] of Object.entries(rawParams))
          if (allowed.has(k)) params[k] = v;
        ...
        for (const p of schema.params) {
          if (p.enumValues && params[p.name] !== undefined) {
            ...
            if (typeof v === "boolean") {
              params[p.name] = v
                ? p.enumValues.includes("on")
                  ? "on"
                  : p.enumValues[0]
                : p.enumValues.includes("off")
                  ? "off"
                  : "";
              continue;
            }
            if (typeof v !== "string" || !p.enumValues.includes(v)) {
              return {
                error: `Invalid value for ${node_type}.${p.name}: ${JSON.stringify(v)}. Must be one of [${p.enumValues.filter(Boolean).join(", ")}].`,
              };
            }
          }
        }

        const r = opts.applyPatch({
          op: "add_node",
          nodeType: node_type,
          id,
          params: params as Partial<VeniceNodeData>,
        });
      ...
      case "pick_model": {
        ...
        const prefer =
          typeof args.prefer === "string" ? args.prefer : undefined;
    ...
    return {
      error:
        "Tool execution failed. Please check your arguments and try again.",
    };
  }
}
...
export async function runAgentTools(opts: RunOptions): Promise<RunResult> {
  const messages: ChatMessage[] = [
    ...
    {
      role: "user",
      content: `${describeDraft(opts.draft)}\n\nUser request: ${opts.userMessage}`,
    },
  ];
    ...
    if (calls.length === 0) {
      ...
      return {
        say:
          ...
          `Made ${toolCallCount} edit${toolCallCount === 1 ? "" : "s"}.`,
        tool_calls: toolCallCount,
        asked_user: false,
      };
    }
