...
const conversationMemoryState = new Map<
  string,
  ConversationMemoryRuntimeState
>();
...
useChatStore.subscribe((state, prevState) => {
  if (
    !prevState ||
    state.conversations.length >= prevState.conversations.length
  )
    return;
...
async function pullMemoryContextForSend(
  userMessage: string,
  conversationId: string,
  characterId?: string,
) {
...
function stopTtsWhenStartingReply(): void {
  if (
    useSettingsStore.getState().audioPreferences?.chatTts.stopOnNewReply !==
    true
  )
    return;
...
export type ChatMemoryStatus =
  "disabled" | "idle" | "loading" | "injected" | "failed";
export type ChatMemoryDecision =
  | {
      mode: "auto";
      source?: "global" | "chat_toggle" | "preview" | "prior_context";
    }
  | {
...
export function useChat() {
  ...
  const characterSceneGenerationMode = useSettingsStore(
    (s) => s.characterSceneGenerationMode,
  );
  ...
  const runSceneGeneration = useCallback(
    async (
      ...
      const messageIndex = conv.messages.findIndex(
        (m) => m.id === (assistantMessageId ?? selectedMessageId),
      );
      ...
      if (messageIndex >= 0) {
        setMessageMetadata(convId, messageIndex, {
          sceneGeneration: initialResult,
        });
      }
  ...
  const executeMediaTools = useCallback(
    async (convId: string, model: string) => {
      const conv = useChatStore
        .getState()
        .conversations.find((c) => c.id === convId);
      ...
      const mediaCalls = lastMsg.tool_calls.filter(
        (tc) => tc.function.name === "media_generate_image",
      );
        ...
        try {
          ...
          const payload = {
            ...
            prompt:
              args.prompt ||
              translateRuntime(
                "runtimeGenerated.hooks.useChat.metadata.generateAnImage",
                "Generate an image",
              ),
          ...
          const res = await venice<{ images?: string[] }>("/image/generate", {
            method: "POST",
            body: payload,
          });
  ...
  const maybeAutoGenerateScene = useCallback(
    async (convId: string) => {
      if (
        ...
        characterSceneGenerationMode !== "auto"
      )
        return;
      const conv = useChatStore
        .getState()
      ...
      if (assistantIndex >= 0 && displayText !== text) {
        setMessageMetadata(convId, assistantIndex, {
          sceneGeneration: {
            requestId: "pending",
            status: "queued",
            updatedAt: new Date().toISOString(),
  ...
  const send = useCallback(
    async (
      ...
      const { enableMemoryRetrieval, showPulledContextBeforeSending } =
      ...
      const streamModel =
        isCharacterConversation && conv?.model ? conv.model : model;
      ...
      if (memoryDecision.mode === "approved_context") {
        ...
        !memoryState.previewShown &&
        !memoryState.previewAttempted
      ) {
        setMemoryStatus("loading");
        const requestId = generateId();
        memoryRequestRef.current = { conversationId: convId, requestId };
        setActiveRequestId(convId, requestId);
        const res = await pullMemoryContextForSend(
          userMessage,
          convId,
          characterId,
        );
        const requestStillActive =
          memoryRequestRef.current?.requestId === requestId &&
        ...
        if (res.ok && res.context && res.context.injectedText) {
          ...
          toast.warn(
            translateRuntime(
              "runtimeGenerated.hooks.useChat.notification.memoryRetrievalUnavailable",
              "Memory retrieval unavailable",
            ),
            translateRuntime(
              "runtimeGenerated.hooks.useChat.notification.sendingYourMessageWithoutMemoryContext",
              "Sending your message without memory context.",
            ),
          );
        } else {
          setMemoryStatus("idle");
      ...
      const metadata = contextToInject
        ? {
            ...
            injectedContextSource: contextSource || "mixed",
          }
        : undefined;
