...
describe("CommandPalette", () => {
  ...
  it("routes tab commands through canonical TAB_REGISTRY ids", () => {
    ...
    act(() => {
      fireEvent.click(
        screen.getByRole("button", { name: /^Image Studio\s*Generate$/i }),
      );
    });
  ...
  it("removes its global keyboard listener on unmount", () => {
    ...
    const view = render(
      <CommandPalette open={false} onClose={vi.fn()} onToggle={onToggle} />,
    );
    view.unmount();
    fireEvent.keyDown(window, { key: "k", metaKey: true });
...
describe("CommandPalette — keyboard navigation", () => {
  ...
  it("wraps from last to first with ArrowDown and first to last with ArrowUp", async () => {
    ...
    fireEvent.keyDown(screen.getByRole("dialog"), { key: "ArrowUp" });
    await vi.waitFor(() =>
      expect(getActiveItem()!.id).toBe(items[items.length - 1].id),
    );
    fireEvent.keyDown(screen.getByRole("dialog"), { key: "ArrowDown" });
    await vi.waitFor(() => expect(getActiveItem()!.id).toBe(items[0].id));
  });
  ...
  it("activates the active item on Enter", async () => {
    ...
    await vi.waitFor(() =>
      expect(useSettingsStore.getState().activeTab).toBe(firstTab.id),
    );
  });
  ...
  it("resets active item to first on query change", async () => {
    ...
    fireEvent.keyDown(screen.getByRole("dialog"), { key: "End" });
    await vi.waitFor(() =>
      expect(getActiveItem()!.id).toBe(allItems()[allItems().length - 1].id),
    );
...
describe("CommandPalette — Phase 2B selection-aware Media Studio commands", () => {
  ...
  it("renders media commands once handlers are registered", () => {
    ...
    render(<CommandPalette open onClose={vi.fn()} onToggle={vi.fn()} />);
    expect(
      screen.getByTestId("command-palette-media-section"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("command-palette-select-all"),
    ).toBeInTheDocument();
  });
  ...
  it("Compare requires 2..4 selected", () => {
    ...
    expect(
      (screen.getByTestId("command-palette-compare") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    // 2 selected → enabled
    act(() => {
  ...
  it("Export requires at least one selected", () => {
    ...
    render(<CommandPalette open onClose={vi.fn()} onToggle={vi.fn()} />);
    expect(
      (screen.getByTestId("command-palette-export") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    act(() => {
      useMediaSelectionStore.getState().selectMedia("a");
...
describe("CommandPalette — Phase 2F RP Studio commands", () => {
  ...
  it("renders RP Studio section and Open button", () => {
    ...
    expect(
      screen.getByTestId("command-palette-open-rp-studio"),
    ).toBeInTheDocument();
  });
  ...
  it("Create ST Card creates a local-only draft and routes to rp-studio", async () => {
    const createBlank = vi
      .spyOn(studioHandoff, "createBlankCharacterCardDraft")
  ...
  it("Start Chat calls startChatForCharacter with active card id", async () => {
    ...
    await act(async () => {
      fireEvent.click(
        screen.getByTestId("command-palette-start-character-chat"),
      );
    });
...
describe("CommandPalette — Phase 2H Storage / Privacy commands", () => {
  ...
  it("Refresh Storage Inventory calls the store action", async () => {
    const refreshInventory = vi
      .spyOn(useStoragePrivacyStore.getState(), "refreshInventory")
  ...
  it("Copy Safe Privacy Summary calls the store action", async () => {
    const copySafeSummary = vi
      .spyOn(useStoragePrivacyStore.getState(), "copySafeSummary")
      .mockResolvedValue(undefined);
    render(<CommandPalette open onClose={vi.fn()} onToggle={vi.fn()} />);
    await act(async () => {
      fireEvent.click(
        screen.getByTestId("command-palette-copy-safe-privacy-summary"),
      );
    });
  ...
  it("Export Safe Privacy Summary calls the store action", () => {
    const exportSafeSummary = vi
      .spyOn(useStoragePrivacyStore.getState(), "exportSafeSummary")
      .mockReturnValue(undefined);
    render(<CommandPalette open onClose={vi.fn()} onToggle={vi.fn()} />);
    act(() => {
      fireEvent.click(
        screen.getByTestId("command-palette-export-safe-privacy-summary"),
      );
    });
...
describe("CommandPalette — Phase 5 Prompt Library import reconciliation", () => {
  ...
  it("imports prompts with reconcile=true and surfaces imported + synced counts", async () => {
    ...
    const importPrompts = vi
      .spyOn(usePromptLibraryStore.getState(), "importPrompts")
      .mockResolvedValue({
        imported: [
          { id: "p-new", title: "New", kind: "general", scope: "global" },
        ] as never[],
        reconciled: [
          {
            id: "p-existing",
    ...
    await act(async () => {
      ...
      input =
        createElementSpy.mock.results.find((r) => r.value?.type === "file")
          ?.value ?? null;
    });
    ...
    expect(importPrompts).toHaveBeenCalledWith(expect.anything(), {
      reconcile: true,
    });
    expect(success).toHaveBeenCalledWith(
      expect.stringMatching(/imported 1 prompt/),
    );
    expect(success).toHaveBeenCalledWith(
      expect.stringMatching(/synced 1 prompt/),
    );
  });
});
