...
export function PlaygroundView() {
  ...
  const setPlaygroundAgentModel = useSettingsStore(
    (s) => s.setPlaygroundAgentModel,
  );
  ...
  const handleRun = async () => {
    ...
    const initial: Record<
      string,
      { nodeId: string; status: "pending"; output: undefined; error: undefined }
    > = {};
    ...
    try {
      ...
      toast.success(
        tRuntime(
          "runtimeGenerated.components.playground.playgroundView.notification.workflowCompleted",
        ),
      );
    } catch (err) {
      ...
      if (message !== "Workflow is already running.") {
        toast.fromError(
          err,
          tRuntime(
            "runtimeGenerated.components.playground.playgroundView.notification.workflowFailed",
          ),
        );
      }
    } finally {
  ...
  const handleReset = async () => {
    const shouldClear = await askDecision({
      title: tRuntime(
        "runtimeGenerated.components.playground.playgroundView.metadata.clearPlayground",
      ),
        ...
        <div className="flex items-center justify-between gap-2 px-3 h-11 border-b border-border/50 bg-surface shrink-0">
          ...
          <div className="flex items-center gap-1.5 shrink-0 min-w-0">
            <AgentModelPicker
              ...
              onChange={setPlaygroundAgentModel}
            />
            <button
              onClick={handleReset}
            ...
            >
              <Trans i18nKey="common:surface.componentsPlaygroundPlaygroundView.action.clear" />
            </button>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {linkedWorkflow ? (
              <div className="flex items-center gap-1 min-w-0 px-2 py-0.5 rounded bg-surface-elevated border border-border">
                ...
                <span
                  className="text-[12.5px] text-text-secondary truncate max-w-[180px]"
                  title={linkedWorkflow.name}
                >
                  {linkedWorkflow.name}
                </span>
                <button
            ...
            <span className="text-[12px] text-text-muted font-mono shrink-0">
              {draft.nodes.length}
              <Trans i18nKey="common:surface.componentsPlaygroundPlaygroundView.text.n" />{" "}
              {draft.edges.length}e
            </span>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              title={
                validation.errors.length > 0
                  ...
                  : tRuntime(
                      "runtimeGenerated.components.playground.playgroundView.attribute.runTheWorkflowInline",
                    )
              }
            >
              {isRunning ? (
                <>
                  ...
                  <span className="hidden sm:inline">
                    <Trans i18nKey="common:surface.componentsPlaygroundPlaygroundView.text.running" />
                  </span>
                </>
              ) : (
                <>
            ...
            <button
              ...
              title={
                linkedWorkflow
                  ? tRuntime(
                      "runtimeGenerated.components.playground.playgroundView.attribute.updateValue1",
                      { value1: linkedWorkflow.name },
                    )
                  : tRuntime(
                      "runtimeGenerated.components.playground.playgroundView.attribute.saveAsNewWorkflow",
                    )
              }
            >
              {linkedWorkflow
                ? tRuntime(
                    "runtimeGenerated.components.playground.playgroundView.text.update",
                  )
                : tRuntime(
                    "runtimeGenerated.components.playground.playgroundView.text.save",
                  )}
            </button>
            {linkedWorkflow && (
              <button
            ...
            >
              <span className="hidden md:inline">
                <Trans i18nKey="common:surface.componentsPlaygroundPlaygroundView.text.openInWorkflows" />
              </span>
              <span className="md:hidden">
                <Trans i18nKey="common:surface.componentsPlaygroundPlaygroundView.text.open" />
              </span>
              <svg
                width="11"
                height="11"
