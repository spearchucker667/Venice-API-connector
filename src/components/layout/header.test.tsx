...
describe("Header component", () => {
  ...
  it("renders correctly for every TAB_IDS entry", () => {
    for (const tabId of TAB_IDS) {
      ...
      const expectedTitle = i18n.t(
        `navigation:tabs.${tabDesc?.id ?? tabId}.label`,
      );
      ...
      expect(subtitleElement?.textContent).toBe(
        i18n.t(`navigation:tabs.${tabDesc?.id ?? tabId}.subtitle`),
      );

      // Check model selector presence
      const hasSelector =
        !!tabDesc?.modelType && tabDesc.modelSelectorOwner !== "view";
  ...
  it("does not rerender for 100 raw assistant-content mutations", () => {
    modelsData.value = Array.from({ length: 500 }, (_, index) => ({
      id: `model-${index}`,
      name: `Model ${index}`,
    }));
    ...
    act(() => {
      for (let index = 0; index < 100; index += 1) {
        useChatStore
          .getState()
          .appendAssistantStreamDelta(id, { content: "x" });
      }
    });
