  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (
        ...
        listRef.current &&
        !listRef.current.closest(".mesh-panel")?.contains(e.target as Node)
      )
        setOpen(false);
    };
    ...
    const update = () => {
      if (ref.current) {
        ...
        if (listRef.current) {
          ...
          setFlip(
            r.bottom + listHeight + 10 > window.innerHeight &&
              r.top - listHeight - 10 > 0,
          );
        } else {
  ...
  const handleKeyDown = (e: React.KeyboardEvent | KeyboardEvent) => {
    ...
    if (
      ...
      const nextIndex = options.findIndex(
        (o, i) => i > start && o.label.toLowerCase().startsWith(char),
      );
      if (nextIndex >= 0) {
        setHighlightedIndex(nextIndex);
      } else {
        const wrapIndex = options.findIndex((o) =>
        ...
        createPortal(
          ...
          >
              ...
              <div className="p-1 border-b border-border/50">
                <input
                  ...
                  placeholder={tRuntime(
                    "runtimeGenerated.components.ui.select.attribute.search",
                  )}
                  aria-label={tRuntime(
                    "runtimeGenerated.components.ui.select.attribute.searchOptions",
                  )}
            ...
            <div
              ...
              tabIndex={-1}
              className="max-h-60 overflow-y-auto p-0.5"
            >
              {filtered.length === 0 ? (
                <div className="px-2.5 py-2.5 text-[14px] text-text-muted text-center">
                  <Trans i18nKey="common:surface.componentsUiSelect.text.noResults" />
                </div>
              ) : (
                filtered.map((o, i) => (
                  <div
