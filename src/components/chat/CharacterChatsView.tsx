          ...
          <div className="flex h-full min-h-0 flex-col items-center justify-center p-6 md:p-10">
            {localCards.length > 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 max-h-[60vh] overflow-y-auto pr-1">
                  {filteredLocalCardsForEmpty.map((card) => {
                    ...
                    return (
                      <article
                        key={card.id}
                        className="flex flex-col justify-between rounded-xl border border-border/50 bg-surface-elevated/40 p-4 soft-panel hover:border-accent/50 transition-all"
                      >
                          ...
                          {card.tags && card.tags.length > 0 && (
                            <div className="flex flex-wrap gap-1">
                              {card.tags.slice(0, 3).map((tag) => (
                                <span
                                  key={tag}
                                  className="rounded bg-surface px-2 py-0.5 text-[11px] text-text-muted border border-border/30"
                                >
                                  {tag}
                                </span>
                              ))}
              ...
              <div className="mesh-card max-w-md rounded-2xl p-8 text-center">
                <div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-accent/10 text-accent">
                  <Users size={22} />
                </div>
                ...
                <div className="mt-5 flex flex-wrap justify-center gap-2">
                  <button
                    type="button"
                    onClick={handleBrowseHosted}
                    className="rounded-lg bg-accent px-4 py-2 text-[13px] font-semibold text-accent-fg hover:bg-accent-hover focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => void handleCreateNewLocalCard()}
                    className="rounded-lg border border-border bg-surface px-4 py-2 text-[13px] font-semibold text-text-secondary hover:text-text-primary"
                  >
