...
export function CharacterEditor({ cardId, onClose, disabled = false }: Props) {
  ...
  const [activeStudioStep, setActiveStudioStep] = useState<number | "all">("all");
  ...
  const isStepVisible = (stepIndex: number) => activeStudioStep === "all" || activeStudioStep === stepIndex;
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
        ...
        <p className="text-[12px] text-text-muted">
          {typeof activeStudioStep === "number"
            ? `Step ${activeStudioStep + 1} of ${studioSteps.length}: ${studioSteps[activeStudioStep]}. Your local draft remains autosaved while you move between steps.`
            : "Showing all steps (1-10). Select a step tab above to focus on specific fields."}
        </p>
