...
describe("Tab registry (BUG-TAB-ALIASES regression)", () => {
  ...
  it("keeps Chat, Character Chats, and Characters as distinct canonical destinations", () => {
    expect(TAB_REGISTRY.find((tab) => tab.id === "chat")?.id).toBe("chat");
    expect(TAB_REGISTRY.find((tab) => tab.id === "character-chats")?.id).toBe(
      "character-chats",
    );
    expect(TAB_REGISTRY.find((tab) => tab.id === "characters")?.id).toBe(
      "characters",
    );
  });
