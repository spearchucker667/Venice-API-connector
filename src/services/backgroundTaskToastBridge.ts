  ...
  useBackgroundTaskStore.subscribe((state) => {
    ...
    for (const [taskId, task] of Object.entries(currentTasks)) {
      ...
      if (task.status === "queued") {
        if (!prevTask || prevTask.status !== "queued") {
          toast.upsertToast(dedupeKey, {
            ...
            description: translateRuntime(
              "runtimeGenerated.services.backgroundtasktoastbridge.metadata.queued",
              "Queued...",
            ),
            persistent: true,
            actions: [
              {
                ...
                label: translateRuntime(
                  "runtimeGenerated.services.backgroundtasktoastbridge.metadata.cancel",
                  "Cancel",
                ),
                kind: "cancel-task",
                onClick: () =>
        ...
        if (
          !prevTask ||
          prevTask.status !== "processing" ||
          prevTask.progress !== task.progress ||
          prevTask.stage !== task.stage
        ) {
          const stageDescription =
            task.type === "video" && task.stage === "retrieving"
              ? "Retrieving video..."
        ...
        if (prevTask && prevTask.status !== "completed") {
          toast.upsertToast(dedupeKey, {
            ...
            title,
            description: translateRuntime(
              "runtimeGenerated.services.backgroundtasktoastbridge.metadata.completedSuccessfully",
              ...
              ? [
                  {
                    id: "open",
                    label: translateRuntime(
                      "runtimeGenerated.services.backgroundtasktoastbridge.metadata.open",
                      "Open",
                    ),
                    kind: "open-task",
                    onClick: () => {
        ...
        if (prevTask && prevTask.status !== task.status) {
          toast.upsertToast(dedupeKey, {
            ...
            title: translateRuntime(
              "runtimeGenerated.services.backgroundtasktoastbridge.metadata.titleFailed",
              "{{title}} failed",
              { title: title },
            ),
            description:
              task.error ||
              translateRuntime(
                "runtimeGenerated.services.backgroundtasktoastbridge.metadata.anErrorOccurred",
                "An error occurred.",
              ),
            persistent: true,
            actions: [
                ...
                ? [
                    {
                      id: "retry",
                      label: translateRuntime(
                        "runtimeGenerated.services.backgroundtasktoastbridge.metadata.retry",
                        "Retry",
                      ),
                ...
                : []),
              {
                id: "dismiss",
                label: translateRuntime(
                  "runtimeGenerated.services.backgroundtasktoastbridge.metadata.dismiss",
                  "Dismiss",
                ),
                kind: "dismiss",
                onClick: () => {
