...
export function buildResearchSummary(
      ...
      if (src) {
        ...
        warnings.push({
          ...
          message: translateRuntime(
            "runtimeGenerated.services.researchsummaries.metadata.sourceIdNotFoundInSession",
            "Source {{id}} not found in session",
            { id: id },
          ),
        });
      }
  ...
  if (findingIds && findingIds.length > 0) {
    for (const id of findingIds) {
      ...
      if (finding) {
        ...
        warnings.push({
          id: `missing-finding-${id}`,
          severity: "warning",
          message: translateRuntime(
            "runtimeGenerated.services.researchsummaries.metadata.findingIdNotFoundInSession",
            "Finding {{id}} not found in session",
            { id: id },
          ),
        });
      }
  ...
  return {
    title: translateRuntime(
      "runtimeGenerated.services.researchsummaries.metadata.researchSummaryValue1",
      "Research Summary: {{value1}}",
