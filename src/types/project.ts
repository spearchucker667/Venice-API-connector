...
export interface RecipeCompatIssue {
  ...
  kind:
    "field-dropped" | "field-defaulted" | "field-truncated" | "model-unknown";
}
 ...
 * Per polish target B.
 */
export function extractGenerationRecipe(
  item: MediaItem,
...
export function sanitizeRecipeForModel(
  ...
  if (capabilities.supportsVariants) {
    if (typeof out.variants === "number")
      out.variants = Math.max(1, Math.min(4, Math.trunc(out.variants)));
  } else {
    delete out.variants;
  }
  ...
  if (capabilities.dimensionMode === "widthHeight") {
    ...
    if (
      !supportedPair(out.width, out.height, capabilities.widthHeightOptions)
    ) {
    ...
    delete out.height;
    const allowedRatios =
      capabilities.aspectRatios?.map((option) => option.id) ?? [];
    ...
    if (capabilities.dimensionMode === "aspectResolution") {
      const allowedResolutions =
        capabilities.resolutions?.map((option) => option.id) ?? [];
...
export function recipeToImageFormState(
  recipe: GenerationRecipe,
  ...
  if (recipe.negativePrompt !== undefined)
    draft.negativePrompt = recipe.negativePrompt;
...
export function createRecipeHandoff(
  ...
  return {
    ...
    parentMediaId:
      mode === "use" ? null : (handoffRecipe.sourceMediaId ?? null),
  };
}
...
export function getRecipeCompatibilityReport(
  ...
  if (!recipe.prompt || recipe.prompt.trim() === "") {
    issues.push({
      ...
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.recipeIsMissingAPrompt",
        "Recipe is missing a prompt.",
      ),
      severity: "blocker",
  ...
  if (!recipe.model) {
    issues.push({
      field: "model",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.recipeIsMissingATargetModel",
        "Recipe is missing a target model.",
      ),
      severity: "blocker",
      kind: "field-truncated",
    });
  ...
  if (!modelIsKnown) {
    issues.push({
      field: "model",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.modelValue1IsNotInTheKnownRegistryDefaultsApplied",
        'Model "{{value1}}" is not in the known registry — defaults applied.',
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "model-unknown",
    });
  ...
  if (
    ...
    issues.push({
      ...
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportANegativePromptItWillBe",
        "{{value1}} does not support a negative prompt — it will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "field-dropped",
  ...
  if (
    ...
    issues.push({
      field: "seed",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportASeedItWillBeDropped",
        "{{value1}} does not support a seed — it will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "field-dropped",
  ...
  if (!capabilities.supportsVariants && recipe.variants !== undefined) {
    issues.push({
      field: "variants",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportMultipleVariantsValueWillBeDropped",
        "{{value1}} does not support multiple variants — value will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "field-dropped",
  ...
  if (capabilities.supportsSteps === false && recipe.steps !== undefined) {
    issues.push({
      field: "steps",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportStepsValueWillBeDropped",
        "{{value1}} does not support steps — value will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "field-dropped",
  ...
  if (
    ...
    issues.push({
      field: "cfgScale",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportCfgScaleValueWillBeDropped",
        "{{value1}} does not support CFG scale — value will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "field-dropped",
  ...
  if (capabilities.supportsStyle === false && recipe.style !== undefined) {
    issues.push({
      field: "style",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportAStylePresetValueWillBe",
        "{{value1}} does not support a style preset — value will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "warn",
      kind: "field-dropped",
  ...
  if (
    ...
    issues.push({
      field: "aspectRatio",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1UsesPixelSizingAspectRatioWillBeDropped",
        "{{value1}} uses pixel sizing — aspect ratio will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "info",
      kind: "field-dropped",
  ...
  if (
    ...
    issues.push({
      field: "resolution",
      message: translateRuntime(
        "runtimeGenerated.types.project.metadata.value1DoesNotSupportAResolutionPresetValueWillBe",
        "{{value1}} does not support a resolution preset — value will be dropped.",
        { value1: capabilities.modelId },
      ),
      severity: "info",
      kind: "field-dropped",
  ...
  if (capabilities.dimensionMode === "widthHeight") {
    ...
    if (
      ...
      originalPair !== finalPair
    ) {
      issues.push({
        field: "width/height",
        message: translateRuntime(
          "runtimeGenerated.types.project.metadata.dimensionsOriginalpairAreNotSupportedDefaultedToFinalpair",
          "Dimensions {{originalPair}} are not supported — defaulted to {{finalPair}}.",
          { originalPair: originalPair, finalPair: finalPair },
        ),
        severity: "info",
    ...
    if (
      ...
      issues.push({
        field: "aspectRatio",
        message: translateRuntime(
          "runtimeGenerated.types.project.metadata.aspectRatioValue1IsNotSupportedDefaultedToValue2",
          'Aspect ratio "{{value1}}" is not supported — defaulted to {{value2}}.',
          { value1: recipe.aspectRatio, value2: sanitizedRecipe.aspectRatio },
        ),
        severity: "info",
        kind: "field-defaulted",
      });
    ...
    if (capabilities.dimensionMode === "aspectResolution") {
      ...
      if (
        ...
        issues.push({
          field: "resolution",
          message: translateRuntime(
            "runtimeGenerated.types.project.metadata.resolutionValue1IsNotSupportedDefaultedToValue2",
            'Resolution "{{value1}}" is not supported — defaulted to {{value2}}.',
            { value1: recipe.resolution, value2: sanitizedRecipe.resolution },
          ),
          severity: "info",
          kind: "field-defaulted",
        });
