...
export function sanitizeWorkflowStep(input: unknown): WorkflowStep {
  ...
  const id =
    typeof obj.id === "string" ? obj.id.slice(0, 100) : crypto.randomUUID();
  const kind = isValidWorkflowStepKind(obj.kind) ? obj.kind : "note";
  const title =
    typeof obj.title === "string" ? obj.title.slice(0, 200) : "Untitled Step";
  const description =
  ...
  const order =
    typeof obj.order === "number" && !isNaN(obj.order) ? obj.order : 0;
  ...
  if (obj.ref && typeof obj.ref === "object") {
    ...
    ref = {
      promptId:
        typeof r.promptId === "string" ? r.promptId.slice(0, 100) : undefined,
      promptVersionId:
        typeof r.promptVersionId === "string"
          ? r.promptVersionId.slice(0, 100)
          : undefined,
      sceneId:
        typeof r.sceneId === "string" ? r.sceneId.slice(0, 100) : undefined,
      sceneVersionId:
        typeof r.sceneVersionId === "string"
          ? r.sceneVersionId.slice(0, 100)
          : undefined,
      mediaId:
        typeof r.mediaId === "string" ? r.mediaId.slice(0, 100) : undefined,
      recipeId:
        typeof r.recipeId === "string" ? r.recipeId.slice(0, 100) : undefined,
      characterId:
        typeof r.characterId === "string"
          ? r.characterId.slice(0, 100)
          : undefined,
      scenarioId:
        typeof r.scenarioId === "string"
          ? r.scenarioId.slice(0, 100)
          : undefined,
      projectId:
        typeof r.projectId === "string" ? r.projectId.slice(0, 100) : undefined,
    };
  }
  ...
  const outputKey =
    typeof obj.outputKey === "string" ? obj.outputKey.slice(0, 100) : undefined;
...
export function sanitizeWorkflowVersion(input: unknown): WorkflowVersion {
  ...
  const workflowId =
    typeof obj.workflowId === "string"
      ? obj.workflowId.slice(0, 100)
      : crypto.randomUUID();
  const version =
    typeof obj.version === "number" && !isNaN(obj.version) ? obj.version : 1;
  const title =
    typeof obj.title === "string" ? obj.title.slice(0, 200) : "Version 1";
  const notes =
    typeof obj.notes === "string" ? obj.notes.slice(0, 2000) : undefined;
  const createdAt =
  ...
  if (obj.source && typeof obj.source === "object") {
    ...
    const sourceId =
      typeof s.sourceId === "string" ? s.sourceId.slice(0, 100) : undefined;
    source = {
      type: type as NonNullable<WorkflowVersion["source"]>["type"],
      sourceId,
    };
  }
...
export function sanitizeWorkflowTemplateItem(
  input: unknown,
  ...
  const id =
    typeof obj.id === "string" ? obj.id.slice(0, 100) : crypto.randomUUID();
  const scope =
    obj.scope === "global" || obj.scope === "project" ? obj.scope : "project";
  const projectId =
    typeof obj.projectId === "string" ? obj.projectId.slice(0, 100) : null;
  const title =
...
export function createWorkflowTemplateItem(
  input: Partial<WorkflowTemplateItem>,
): WorkflowTemplateItem {
  const versions =
    input.versions && input.versions.length > 0
      ? input.versions
      : [
          createWorkflowVersion({
            workflowId: input.id || "temp",
            version: 1,
            title: translateRuntime(
              "runtimeGenerated.types.workflow.metadata.initialVersion",
              "Initial Version",
            ),
          }),
        ];
  ...
  const currentVersionId =
    input.currentVersionId || versions[versions.length - 1].id;
...
export function exportWorkflowTemplateItems(
...
export function parseWorkflowTemplateImport(
  input: unknown,
  ...
  if (envelope.app !== "Venice Forge") {
    result.skipped.push({
      reason: "Invalid import format: missing or incorrect app signature",
    });
    return result;
  }
  ...
  if (envelope.version !== WORKFLOW_TEMPLATE_VERSION) {
    result.skipped.push({
      reason: `Unsupported version: expected ${WORKFLOW_TEMPLATE_VERSION}, got ${envelope.version}`,
    });
    return result;
  }
  ...
  if (!Array.isArray(envelope.workflows)) {
    result.skipped.push({
      reason: "Invalid import format: workflows is not an array",
    });
    return result;
  }
  ...
  for (const rawItem of envelope.workflows) {
    const title =
      rawItem && typeof rawItem === "object" && "title" in rawItem
    ...
    try {
      ...
      if (isPromptSecretLike(jsonStr)) {
        result.skipped.push({
          title,
          reason:
            "Contains secret-like material (blocked by security heuristic)",
        });
        continue;
      }
      if (
        jsonStr.includes("exec(") ||
        jsonStr.includes("eval(") ||
        jsonStr.includes("child_process")
      ) {
        result.skipped.push({
          title,
          reason:
            "Contains executable code patterns (blocked by security heuristic)",
        });
        continue;
      }
      ...
      const resultWithItems = result as WorkflowImportResult & {
        _items?: WorkflowTemplateItem[];
      };
...
function isValidWorkflowStepTarget(
  target: unknown,
): target is WorkflowStepTarget {
