...
function validateSchema(
  ...
  if (Array.isArray(schema.anyOf)) {
    if (
      !schema.anyOf.some((candidate) => {
        try {
          validateSchema(value, candidate as Record<string, unknown>, label);
          return true;
        } catch {
          return false;
        }
  ...
  if (schema.type === "object") {
    ...
    for (const key of required)
      if (!Object.hasOwn(object, key)) throw new Error(`Missing ${key}.`);
    ...
    for (const [key, item] of Object.entries(object))
      if (properties[key]) validateSchema(item, properties[key], key);
  }
}

function simpleValidator(
  schema: Record<string, unknown>,
  options?: { relativePathKeys?: readonly string[] },
): RuntimeValidator<Record<string, unknown>> {
  return {
    parse(input) {
      const record = assertPlainObject(input);
      validateSchema(record, schema);
      for (const key of options?.relativePathKeys ?? [])
        boundedString(record, key, 500, key === "relativeDirectory");
...
const DEFINITIONS: Array<{
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.get",
    description: "Read a bounded segment of a managed document.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "document:read",
    approval: "never",
    required: ["documentId"],
    properties: {
      documentId: stringId,
      revisionId: { anyOf: [stringId, { type: "null" }] },
      cursor: { anyOf: [{ type: "string", maxLength: 512 }, { type: "null" }] },
    },
  },
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.proposeEdits",
    description:
      "Prepare deterministic managed-document edits without writing.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "document:propose-update",
    approval: "always",
    required: ["documentId", "baseRevisionId", "summary", "operations"],
    properties: {
      documentId: stringId,
      baseRevisionId: stringId,
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.create",
    description: "Create a non-overwriting app-managed document.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "document:create",
    approval: "never",
    required: ["projectId", "relativePath", "format", "document", "overwrite"],
    properties: {
      projectId: stringId,
      relativePath,
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.export",
    description: "Request export through a user-confirmed native save dialog.", // i18n-allow: model-facing tool metadata, not application presentation
    ...
    properties: {
      ...
      revisionId: { anyOf: [stringId, { type: "null" }] },
      format: { enum: ["txt", "md", "json", "csv", "html", "docx", "pdf"] },
      suggestedFileName: { type: "string", minLength: 1, maxLength: 255 },
    },
  },
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.getRevision",
    description: "Read bounded content from an immutable revision.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "document:read-revision",
    approval: "never",
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.restoreRevision",
    description: "Propose restoration as a new immutable revision.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "document:restore-revision",
    approval: "always",
    required: [
      "documentId",
      "currentRevisionId",
    ...
    properties: {
      documentId: stringId,
      currentRevisionId: stringId,
      restoreRevisionId: stringId,
      reason: { type: "string", minLength: 1, maxLength: 500 },
    },
  },
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "document.promoteAttachment",
    description:
      "Promote an attached file into a non-overwriting managed document with bounded text extraction, secret redaction, and audit.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "attachment:promote",
    approval: "never",
    required: [
      "projectId",
      "relativePath",
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.list",
    description: "List bounded entries beneath the granted workspace.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "workspace:list",
    approval: "never",
    required: ["workspaceId", "relativeDirectory", "recursive", "maxDepth"],
    properties: {
      workspaceId: stringId,
      relativeDirectory: { type: "string", maxLength: 500 },
      recursive: { type: "boolean" },
      maxDepth: { type: "integer", minimum: 0, maximum: 10 },
    },
    relativePathKeys: ["relativeDirectory"],
  },
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.read",
    description: "Read a supported file beneath the granted workspace.", // i18n-allow: model-facing tool metadata, not application presentation
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.search",
    description: "Search supported text files without shell execution.", // i18n-allow: model-facing tool metadata, not application presentation
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.createFile",
    description: "Prepare creation of a non-overwriting workspace file.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "workspace:create-file",
    approval: "policy",
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.createDirectory",
    description: "Prepare creation of a workspace directory.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "workspace:create-directory",
    approval: "policy",
    required: ["workspaceId", "relativePath"],
    properties: { workspaceId: stringId, relativePath },
    relativePathKeys: ["relativePath"],
  },
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.proposeChangeset",
    description: "Prepare a bounded multi-file workspace changeset.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "workspace:propose-update",
    approval: "policy",
    required: ["workspaceId", "summary", "changes"],
    properties: {
      workspaceId: stringId,
      summary: { type: "string", minLength: 1, maxLength: 500 },
      changes: {
  ...
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.move",
    description: "Propose an explicitly confirmed move within the workspace.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "workspace:move",
    approval: "always",
    required: ["workspaceId", "sourcePath", "destinationPath"],
    properties: {
      workspaceId: stringId,
      sourcePath: relativePath,
      destinationPath: relativePath,
    },
    relativePathKeys: ["sourcePath", "destinationPath"],
  },
  // i18n-allow-next-line: model-facing tool metadata, not application presentation
  {
    internalName: "workspace.trash",
    description: "Propose moving a workspace item to recoverable Trash.", // i18n-allow: model-facing tool metadata, not application presentation
    capability: "workspace:trash",
    approval: "always",
    required: ["workspaceId", "relativePath"],
    properties: { workspaceId: stringId, relativePath },
    relativePathKeys: ["relativePath"],
...
export function createCanonicalToolDefinitions(): RegisteredTool[] {
  return DEFINITIONS.map((definition) => {
    ...
    return {
      ...
      argsValidator: simpleValidator(parameters, {
        relativePathKeys: definition.relativePathKeys,
      }),
    };
  });
}
...
export class ToolRegistry {
  ...
  constructor(
    ...
    for (const tool of tools) {
      if (
        this.byInternal.has(tool.internalName) ||
        this.byProvider.has(tool.providerName)
      ) {
        throw new Error(`Duplicate tool registration: ${tool.internalName}.`);
      }
      if (
        toolNameMap[tool.internalName] !== tool.providerName ||
        !internalToolNameForProvider(tool.providerName)
      ) {
        throw new Error(`Invalid tool name mapping: ${tool.internalName}.`);
      }
      if (
        tool.schema.function.name !== tool.providerName ||
        !tool.argsValidator
      ) {
  ...
  getProviderSchemas(input: {
    ...
    if (
      !this.modelCapabilities.supportsFunctionCalling(input.modelId) ||
      !isGrantActive(input.grant, input.sessionId)
    )
      return [];
