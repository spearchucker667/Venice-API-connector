...
function enforceCacheBound(items: MediaItem[]): MediaItem[] {
  ...
  return items
    ...
    .slice(0, MEDIA_IN_MEMORY_CACHE_MAX);
}
...
function estimateMediaByteSize(image: string | undefined): number {
  ...
  const payload =
    image.startsWith("data:") && commaIndex !== -1
...
export const useMediaStore = create<MediaState>((set, get) => ({
  ...
  refresh: async () => {
    ...
    try {
      const page = await StorageService.getItemsPageWithMeta<unknown>(
        "images",
        { offset: 0, limit: MEDIA_PAGE_SIZE },
      );
  ...
  upsert: async (item, options) => {
    ...
    set((state) => {
      ...
      const next = enforceCacheBound(
        [saved, ...without].sort((a, b) => b.timestamp - a.timestamp),
      );
      return {
        items: next,
  ...
  upsertDerivative: async (item, parentId) => {
    const parent =
      get().items.find((candidate) => candidate.id === parentId) ??
      (await get().loadById(parentId));
    ...
    try {
      const updatedParent = await StorageService.patchMedia<MediaItem>(
        parentId,
        (existing) => ({
      ...
      set((state) => {
        const withoutChild = state.items.filter(
          (existing) => existing.id !== saved.id,
        );
        const items = enforceCacheBound(
          reconcileList([saved, ...withoutChild], [updatedParent]).sort(
        ...
        return {
          ...
          totalCount:
            state.totalCount +
            (state.items.some((existing) => existing.id === saved.id) ? 0 : 1),
        };
      });
      return saved;
    } catch (err) {
      ...
      try {
        ...
        toast.error(
          SAFE_DERIVATIVE_ROLLBACK_ERROR,
          safeDiagnostic(rollbackErr),
        );
        ...
        set((state) => {
          ...
          const items = enforceCacheBound(
            reconcileList([saved, ...withoutChild], []).sort(
              (a, b) => b.timestamp - a.timestamp,
            ),
          );
          return {
            ...
            lastError: SAFE_DERIVATIVE_ROLLBACK_ERROR,
          };
        });
      }
      if (rollbackOk) {
      ...
      set((state) => ({
        items: state.items.map((item) =>
          updatedSet.has(item.id)
            ? { ...item, ...patch, id: item.id, timestamp: item.timestamp }
            : item,
        ),
      }));
    }
  ...
  remove: async (id) => {
    try {
      if (process.env.NODE_ENV !== "test") {
        ...
        const { coerceToChatMediaReferenceArray } =
          await import("../types/conversation");
        ...
        const hasReference = conversations.some((c) =>
          c.messages.some((m) => {
            ...
            return refs.some(
              (r) => r.mediaId === id && r.deletedFromChatAt == null,
            );
          }),
        );
        if (hasReference) {
          const { askDecision } =
            await import("../components/ui/modal-requests");
          const confirmed = await askDecision({
            title: translateRuntime(
              "runtimeGenerated.stores.mediaStore.metadata.deleteMediaInUse",
              "Delete media in use?",
            ),
            detail:
              "This media is currently referenced in one or more chat messages. The message will remain, but the media will be broken. Are you sure you want to delete it?",
  ...
  removeMany: async (ids) => {
    try {
      if (process.env.NODE_ENV !== "test") {
        ...
        const hasReference = ids.some((id) =>
        ...
        if (hasReference) {
          ...
          const confirmed = await askDecision({
            ...
            detail:
              "One or more of these media items are referenced in chat messages. Are you sure you want to delete them?",
            actionLabel: "Delete media",
            danger: true,
          });
  ...
  addTagsMany: async (ids, tags) => {
    ...
    if (failedIds.length > 0) {
      toast.error(
        translateRuntime(
          "runtimeGenerated.stores.mediaStore.notification.tagUpdatePartiallyFailed",
          "Tag update partially failed",
        ),
        translateRuntime(
          "runtimeGenerated.stores.mediaStore.notification.couldNotUpdateTagsForValue1ItemS",
          "Could not update tags for {{value1}} item(s).",
    ...
    set((state) => ({
      items: state.items.map((item) => {
        ...
        return merged.length === item.tags.length
          ? item
          : { ...item, tags: merged };
      }),
    }));
  ...
  removeTagMany: async (ids, tag) => {
    ...
    const results = await Promise.allSettled(
      ids.map(async (id) => {
        const existing = state.items.find((item) => item.id === id);
        if (!existing || !existing.tags.includes(normalized))
          return { id, skipped: true };
    ...
    if (failedIds.length > 0) {
      toast.error(
        translateRuntime(
          "runtimeGenerated.stores.mediaStore.notification.tagRemovalPartiallyFailed",
          "Tag removal partially failed",
        ),
        translateRuntime(
          "runtimeGenerated.stores.mediaStore.notification.couldNotRemoveTagForValue1ItemS",
          "Could not remove tag for {{value1}} item(s).",
          { value1: failedIds.length },
        ),
      );
  ...
  loadById: async (id) => {
    ...
    try {
      ...
      set((state) => {
        ...
        return {
          items: enforceCacheBound(
            [migrated, ...state.items].sort(
              (a, b) => b.timestamp - a.timestamp,
            ),
          ),
        };
...
export const selectById = (state: MediaState, id: string) =>
...
export function filterMedia(
  ...
  switch (filter) {
    ...
    case "upscaled":
      return items.filter(
        (item) => item.operation === "upscale" || item.upscaled === true,
      );
    case "edited":
      return items.filter(
        (item) =>
          item.operation === "edit" || item.operation === "background-remove",
      );
    case "has-recipe":
    ...
    case "has-seed":
      return items.filter(
        (item) => typeof item.seed === "number" && Number.isInteger(item.seed),
      );
    case "no-seed":
      return items.filter(
        (item) => typeof item.seed !== "number" || !Number.isInteger(item.seed),
...
export function sortMedia(items: MediaItem[], sort: MediaSort): MediaItem[] {
  ...
  switch (sort) {
    ...
    case "has-seed":
      out.sort((a, b) => {
        const as =
          typeof a.seed === "number" && Number.isInteger(a.seed) ? 1 : 0;
        const bs =
          typeof b.seed === "number" && Number.isInteger(b.seed) ? 1 : 0;
...
export function searchMedia(items: MediaItem[], query: string): MediaItem[] {
  ...
  return items.filter((item) => {
    if (
      ...
      return true;
    if (typeof item.model === "string" && item.model.toLowerCase().includes(q))
      return true;
    if (typeof item.note === "string" && item.note.toLowerCase().includes(q))
      return true;
    if (
      Array.isArray(item.tags) &&
      item.tags.some((tag) => typeof tag === "string" && tag.includes(q))
    )
      return true;
    return false;
  });
