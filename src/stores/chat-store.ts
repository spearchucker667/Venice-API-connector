...
const fallbackSummaryCollections = new WeakMap<
  Conversation[],
  ConversationSummary[]
>();
...
export function selectConversationSummaries(
  ...
  if (
    state.conversations.length === state.conversationSummaries.length &&
    firstConversationId === firstSummaryId
  ) {
...
export function selectActiveConversationModel(
  state: ChatState,
): string | undefined {
...
export function selectActiveConversationCharacter(
  state: ChatState,
): ConversationCharacterMeta | undefined {
  return state.conversations.find(
    (conversation) => conversation.id === state.activeConversationId,
  )?.metadata?.character;
}
  ...
  getItem: async (name) => {
    try {
      if (
        ...
        window.localStorage /* localStorage-allowed: one-time migration */
      ) {
        const legacy =
          window.localStorage /* localStorage-allowed: one-time migration */.getItem(
            name,
          ); /* localStorage-allowed: one-time migration */
        if (legacy) {
          await StorageService.saveItem("chats", { id: name, value: legacy });
          window.localStorage /* localStorage-allowed: remove migrated legacy copy */.removeItem(
            name,
          ); /* localStorage-allowed: remove migrated legacy copy */
          return legacy;
        }
      }
    } catch (e) {
      logger.warn(
        "[chat-store] Migration from localStorage failed", /* localStorage-allowed: diagnostic only */
        e,
      ); /* localStorage-allowed: diagnostic only */
    }
    try {
      const item = await StorageService.getItem<{ id: string; value: string }>(
        "chats",
        name,
      );
...
interface ChatState {
  ...
  updateMessage: (
    conversationId: string,
    messageId: string,
    update: { content: ConversationMessage["content"]; updatedAt: number },
  ) => boolean;
  ...
  recordGeneratedMediaForMessage: (
    ...
    | { ok: true; ref: ChatMediaReference }
    | { ok: false; ref: ChatMediaReference; error: string };
...
export type ConversationMutationPriority =
  "stream-delta" | "message-boundary" | "structural";
...
function commitConversationMutation(
  ...
  set((state) => {
    ...
    const summaryIndex = conversationSummaries.findIndex(
      (summary) => summary.id === conversationId,
    );
    if (summaryIndex >= 0)
      conversationSummaries[summaryIndex] = toConversationSummary(updated);
    else conversationSummaries.unshift(toConversationSummary(updated));
    return { conversations, conversationSummaries };
  });
  if (updatedConversation)
    markDirtyConversation(conversationId, updatedConversation, priority);
}
...
export function ensureStableMessageIds(
  conversation: Conversation,
): Conversation {
  let changed = false;
  ...
  return changed
    ? touchConversation({ ...conversation, messages }, conversation.updatedAt)
    : conversation;
}
    ...
    (set, get) => ({
      ...
      createConversation: (model) => {
        ...
        const conv: Conversation = {
          ...
          title: translateRuntime(
            "runtimeGenerated.stores.chatStore.metadata.newChat",
            "New Chat",
          ),
          messages: [],
          model,
      ...
      createCharacterConversation: (character, fallbackModel) => {
        ...
        const preferredModel =
          (character.modelId && character.modelId.trim()) ||
          fallbackModel ||
          DEFAULT_CHAT_MODEL;
        const activeProj = useSettingsStore.getState().activeProjectId;
        const conv: Conversation = {
          id,
          title: translateRuntime(
            "runtimeGenerated.stores.chatStore.metadata.chatWithValue1",
            "Chat with {{value1}}",
      ...
      createLocalCharacterConversation: (card, fallbackModel) => {
        ...
        const preferredModel =
          ...
          DEFAULT_CHAT_MODEL;
        const activeProj = useSettingsStore.getState().activeProjectId;
        const compiledSystemPrompt = compileCharacterSystemPrompt(
          card.systemPrompt,
          card.instructions,
        );
        const characterMeta: ConversationCharacterMeta = {
        ...
        const conv: Conversation = {
          ...
          title: translateRuntime(
            "runtimeGenerated.stores.chatStore.metadata.chatWithValue1",
            "Chat with {{value1}}",
            { value1: card.name || "local character" },
          ),
          model: preferredModel,
          systemPrompt: compiledSystemPrompt,
      ...
      setActiveConversation: (id) =>
        set({ activeConversationId: id, pendingContext: null }),
      ...
      deleteConversations: async (ids) => {
        ...
        for (const id of targetIds) {
          try {
            ...
            const error = redactErrorMessage(
              chatRes.error || convRes.error || "Delete failed",
            );
        ...
        const activeConversationDeleted = deletedSet.has(
        ...
        try {
          if (deleted.length > 0) {
            set((s) => ({
              ...
              conversationSummaries: s.conversationSummaries.filter(
                (summary) => !deletedSet.has(summary.id),
              ),
      ...
      restoreConversation: async (conv) => {
        ...
        set((s) => {
          ...
          return {
            ...
            conversationSummaries: [
              toConversationSummary(restored),
              ...s.conversationSummaries,
            ],
          };
        });
      ...
      appendAssistantStreamDelta: (conversationId, delta) => {
        ...
        commitConversationMutation(
          ...
          (c) => {
            ...
            if (delta.providerRequestId)
              metadata.providerRequestId = delta.providerRequestId;
            if (delta.usage) metadata.usage = delta.usage;
            msgs[msgs.length - 1] = {
              ...last,
              content: last.content + (delta.content ?? ""),
              reasoning_content:
                (last.reasoning_content || "") + (delta.reasoning ?? ""),
                  ...
                  useMediaStore.getState().upsert(
                    {
                      ...
                      prompt:
                        typeof m.content === "string"
                          ...
                          : translateRuntime(
                              "runtimeGenerated.stores.chatStore.metadata.chatGeneratedMedia",
      ...
      updateMessage: (conversationId, messageId, update) => {
        ...
        commitConversationMutation(
          ...
          (c) => {
            ...
            return updated
              ? touchConversation({ ...c, messages }, update.updatedAt)
              : c;
          },
          "structural",
        );
        ...
        const fork: Conversation = touchConversation(
          {
            ...
            id: forkId,
            title: translateRuntime(
              "runtimeGenerated.stores.chatStore.metadata.value1Fork",
              "{{value1}} — Fork",
              { value1: source.title },
            ),
            messages,
            ...
            metadata: source.metadata
              ? { ...source.metadata, tags: [...source.metadata.tags] }
              : undefined,
            memory: source.memory
              ? {
                  ...
                  userFacts: source.memory.userFacts.map((fact) => ({
                    ...fact,
                  })),
      ...
      setMessageMetadata: (conversationId, messageIndex, metadataPatch) => {
        commitConversationMutation(
          ...
          (c) => {
            ...
            msgs[messageIndex] = {
              ...msg,
              metadata: { ...msg.metadata, ...metadataPatch },
            };
            return touchConversation({ ...c, messages: msgs });
          },
          "structural",
        ...
        commitConversationMutation(
          ...
          (c) => {
            const msgs = (c.messages ?? []).map((m) => {
              ...
              const existing = coerceToChatMediaReferenceArray(
                m.metadata?.generatedMedia,
              );
            ...
            if (!msgs.some((m) => m.id === messageId)) {
              didSucceed = false;
              logger.warn?.(
                "chat-store: recordGeneratedMediaForMessage: message not found",
                {
                  conversationId,
                  messageId,
                },
      ...
      removeMediaReferenceFromMessage: (conversationId, messageId, refId) => {
        ...
        commitConversationMutation(
          ...
          (c) => {
            const msgs = (c.messages ?? []).map((m) => {
              if (m.id !== messageId) return m;
              const current = coerceToChatMediaReferenceArray(
                m.metadata?.generatedMedia,
              );
      ...
      setSystemPrompt: (prompt) => {
        ...
        if (limitResult.isOverLimit) {
          toast.warn(
            limitResult.message ||
              translateRuntime(
                "runtimeGenerated.stores.chatStore.notification.systemPromptExceedsMaximumLength",
          ...
          toast.warn(
            limitResult.message ||
              translateRuntime(
                "runtimeGenerated.stores.chatStore.notification.systemPromptApproachingLengthLimit",
                "System prompt approaching length limit",
              ),
          );
        }
...
function scheduleFlush(
  ...
  if (priority === "structural") {
    void flushConversationSave(id).catch((error) => {
      logger.error(
        "[chat] structural conversation save failed",
...
async function writeConversation(conv: Conversation): Promise<void> {
  ...
  if (!isElectron()) {
    try {
      await StorageService.saveItem("conversations", {
        ...record,
      ...
      const error = redactErrorMessage(
        err instanceof Error ? err.message : "Web conversation save failed",
      );
  ...
  try {
    ...
    const error = redactErrorMessage(
      chatRes.error || "Fallback chat save failed",
    );
