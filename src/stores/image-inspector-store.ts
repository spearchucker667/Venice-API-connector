...
function errorMessage(error: unknown): string {
  return error instanceof Error
    ? error.message
    : "Image Inspector request failed.";
}

async function persistSession(
  session: ImageInspectorSession,
): Promise<ImageInspectorSession> {
...
async function resolveInputDataUrl(
  input: ImageInspectorInput | undefined,
): Promise<string> {
  if (!input?.mediaId) throw new Error("No durable image media is available.");
  const result = await desktopImageInspector.readMediaDataUrl({
    mediaId: input.mediaId,
  });
  if (!result.ok || !result.result)
    throw new Error(result.error || "Image media could not be read.");
...
interface ImageInspectorState {
  ...
  updateSession: (
    id: string,
    patch: Partial<ImageInspectorSession>,
  ) => Promise<void>;
  ...
  performSearch: (
    provider: "venice-google" | "venice-brave",
    queryOverride?: string,
  ) => Promise<void>;
}

export const useImageInspectorStore = create<ImageInspectorState>(
  (set, get) => ({
    activeSession: null,
    sessions: [],
    loading: false,
    ...
    refreshSessions: async () => {
      set({ loading: true });
      try {
        const sessions = await StorageService.getItems<ImageInspectorSession>(
          "imageInspectorSessions",
        );
    ...
    loadSession: async (id: string) => {
      set({ loading: true });
      try {
        const session = (await StorageService.getItem(
          "imageInspectorSessions",
          id,
        )) as ImageInspectorSession | null;
        if (session) {
          set({ activeSession: session });
        } else {
          toast.error(
            translateRuntime(
              "runtimeGenerated.stores.imageInspectorStore.notification.couldNotLoadSessionId",
      ...
      try {
        const newSession: ImageInspectorSession = {
          ...
          title:
            input.displayName ||
            translateRuntime(
              "runtimeGenerated.stores.imageInspectorStore.metadata.newSession",
              "New Session",
            ),
          status: "draft",
          inputs: [input],
    ...
    deleteSession: async (id: string) => {
      try {
        ...
        set((state) => ({
          ...
          searchResults:
            state.activeSession?.id === id ? [] : state.searchResults,
        }));
      } catch (error) {
        toast.error(errorMessage(error));
      }
    ...
    updateSession: async (
      ...
      try {
        const existing = (await StorageService.getItem(
          "imageInspectorSessions",
          id,
        )) as ImageInspectorSession | null;
        if (!existing) return;
        ...
        set((state) => ({
          ...
          activeSession:
            state.activeSession?.id === id ? saved : state.activeSession,
        }));
      } catch (error) {
        toast.error(errorMessage(error));
      }
    ...
    startAnalysis: async (
      ...
      if (!activeSession) {
        toast.error(
          translateRuntime(
            "runtimeGenerated.stores.imageInspectorStore.notification.loadASessionBeforeStartingAnalysis",
            "Load a session before starting analysis.",
          ),
        );
        return;
      }
        ...
        if (!(error instanceof DOMException && error.name === "AbortError")) {
          ...
          if (currentSession) {
            const failedSession: ImageInspectorSession = {
              ...
              error: {
                code:
                  error instanceof ImageInspectorAnalysisError
                    ? error.code
    ...
    performSearch: async (provider, queryOverride) => {
      ...
      if (!activeSession) {
        toast.error(
          translateRuntime(
            "runtimeGenerated.stores.imageInspectorStore.notification.noActiveSessionForSearch",
            "No active session for search.",
          ),
        );
        return;
      }
      ...
      if (!query) {
        toast.error(
          translateRuntime(
            "runtimeGenerated.stores.imageInspectorStore.notification.noQueryAvailableToSearch",
            "No query available to search.",
          ),
        );
        return;
      }
      ...
      try {
        ...
        for (const src of searchRes.sources) {
          ...
          mappedResults.push({
            ...
            title:
              src.title ||
              translateRuntime(
                "runtimeGenerated.stores.imageInspectorStore.metadata.searchResult",
                "Search Result",
              ),
            ...
            matchReason:
              src.excerpt ||
              `Web search result via ${provider === "venice-google" ? "Google Search" : "Brave Search"}`,
        ...
        if (mappedResults.length > 0) {
          toast.success(
            translateRuntime(
              "runtimeGenerated.stores.imageInspectorStore.notification.foundValue1SearchResultsViaValue2",
              "Found {{value1}} search results via {{value2}}",
              {
                value1: mappedResults.length,
                value2: provider === "venice-google" ? "Google" : "Brave",
              },
            ),
          );
        } else {
          toast.error(
            translateRuntime(
              "runtimeGenerated.stores.imageInspectorStore.notification.noSearchResultsReturnedFromValue1",
              "No search results returned from {{value1}}",
              { value1: provider === "venice-google" ? "Google" : "Brave" },
            ),
          );
        }
      } catch (error) {
