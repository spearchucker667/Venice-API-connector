...
export interface WorkflowTemplateState {
  ...
  addStep(
    workflowId: string,
    step: Omit<WorkflowStep, "id" | "order"> & { order?: number },
  ): Promise<void>;
  updateStep(
    workflowId: string,
    stepId: string,
  ...
  importWorkflows(payload: unknown): Promise<{
    imported: string[];
...
export const useWorkflowTemplateStore = create<WorkflowTemplateState>(
  (set, get) => ({
    workflows: [],
    ...
    ensureWorkflowTemplatesLoaded: async () => {
      ...
      try {
        const raw =
          await StorageService.getItems<WorkflowTemplateItem[]>(
            "workflowTemplates",
          );
        if (raw && Array.isArray(raw)) {
          set({
            workflows: raw.map(sanitizeWorkflowTemplateItem),
            hydrated: true,
          });
        } else {
    ...
    createWorkflow: async (input) => {
      ...
      const projectId =
        input.scope === "global"
          ? null
          : (input.projectId ?? activeProjectId ?? null);
      ...
      const newVersion = createWorkflowVersion({
        ...
        title: translateRuntime(
          "runtimeGenerated.stores.workflowTemplateStore.metadata.initialVersion",
          "Initial Version",
        ),
      ...
      set((s) => ({
        workflows: [newItem, ...s.workflows],
        activeWorkflowId: newItem.id,
      }));
      try {
        ...
        set((s) => ({
          ...
          activeWorkflowId:
            s.activeWorkflowId === newItem.id ? null : s.activeWorkflowId,
          loadError: redactErrorMessage(err),
        }));
        throw err;
      }
    ...
    updateWorkflow: async (workflowId, patch) => {
      ...
      set((s) => ({
        workflows: s.workflows.map((w) => (w.id === workflowId ? next : w)),
      }));
    ...
    addWorkflowVersion: async (workflowId, input) => {
      ...
      const nextVersionNumber =
        Math.max(0, ...current.versions.map((v) => v.version)) + 1;
      ...
      const newVersion = createWorkflowVersion({
        ...
        title:
          input.title ||
          translateRuntime(
            "runtimeGenerated.stores.workflowTemplateStore.metadata.versionNextversionnumber",
            "Version {{nextVersionNumber}}",
    ...
    addStep: async (workflowId, stepInput) => {
      ...
      const currentVersion = current.versions.find(
        (v) => v.id === current.currentVersionId,
      );
      if (!currentVersion) return;

      const steps = [...currentVersion.steps];
      const maxOrder =
        steps.length > 0 ? Math.max(...steps.map((s) => s.order)) : -1;
      const order =
        stepInput.order !== undefined ? stepInput.order : maxOrder + 1;

      const newStep = sanitizeWorkflowStep({
        ...stepInput,
    ...
    updateStep: async (workflowId, stepId, patch) => {
      ...
      const steps = [...currentVersion.steps];
      steps[stepIndex] = sanitizeWorkflowStep({
        ...steps[stepIndex],
        ...patch,
      });
      steps.sort((a, b) => a.order - b.order);
    ...
    deleteWorkflow: async (workflowId) => {
      ...
      const nextActive =
        previousActiveWorkflowId === workflowId
          ? null
    ...
    getWorkflow: (workflowId) =>
      get().workflows.find((w) => w.id === workflowId) || null,

    getCurrentVersion: (workflowId) => {
      const workflow = get().getWorkflow(workflowId);
      if (!workflow) return null;
      return (
        workflow.versions.find((v) => v.id === workflow.currentVersionId) ||
        null
      );
    },
    ...
    importWorkflows: async (payload) => {
      ...
      const importedItems =
        (result as { _items?: WorkflowTemplateItem[] })._items || [];
    ...
    exportWorkflows: (workflowIds) => {
      const workflows = get().workflows.filter((w) =>
